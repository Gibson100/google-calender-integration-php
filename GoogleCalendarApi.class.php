...
class GoogleCalendarApi { 
    ...
    // function for updating calendar events
    public function UpdateCalendarEvent($event_id, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone, $access_token) {
		...
		if($http_code != 200) {
            ...
            $response['type'] = 'error';
            $response['message'] = 'failed to update event event';
        
            return ($response);
        }
        else {
            $response['type'] = 'success';
            $response['message'] = 'successfully updated the event event';
    ...
    public function DeleteCalendarEvent($event_id, $calendar_id, $access_token) {
		...
		if($http_code != 204) {
            ...
            $response['type'] = 'error';
            $response['message'] = 'failed to deleted event';
